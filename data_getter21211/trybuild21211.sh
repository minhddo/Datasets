...
isa211="${ISABELLE_DIR}/Isabelle2021-1/bin/isabelle"
...
# This piece of code is to try to build every single session in the afp-2021 repository, 
...
do
	...
	included_build="-D ../../AFPs/afp-2021/thys/${session_name}"
	import_sessions=""
	outfile="out.txt"
	errfile="err.txt"
	eval "${isa211} build ${import_sessions} ${included_build} > ${outfile} 2> ${errfile}"
	...
	while [ -s "$errfile" ]; do
		...
		extra_import=$(find_substitute "$extra_import" "../afp-getter/substitute_ver2021")
		import_sessions+="-d ../../AFPs/afp-2021/thys/${extra_import} "
