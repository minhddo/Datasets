...
# This piece of code is to try to build every single session in the afp-2022 repository, 
...
do
	if [ "$session_name" = "Query_Optimization" ]; then
		continue
	fi
	...
	included_build="-D ../../AFPs/afp-2022/thys/${session_name}"
	import_sessions=""
	outfile="out.txt"
	errfile="err.txt"
	eval "${isa23} build ${import_sessions} ${included_build} > ${outfile} 2> ${errfile}"
	...
	while [ -s "$errfile" ]; do
		...
		extra_import=$(find_substitute "$extra_import" "../afp-getter/substitute_ver2022")
		if [ "$extra_import" = "Sepref_IICF" ] || [ "$extra_import" = "Sepref_Basic" ] || [ "$extra_import" = "Sepref_Prereq" ]; then
			import_sessions+="-d ../../AFPs/afp-2022/thys/Refine_Imperative_HOL "
		else
			import_sessions+="-d ../../AFPs/afp-2022/thys/${extra_import} "
		fi
